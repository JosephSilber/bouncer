...
class AbilitiesForModelsTest extends BaseTestCase
{
    ...
    public function test_model_wildcard_ability()
    {
        $user1 = User::create();
        $user2 = User::create();
    ...
    public function test_model_wildcard_blanket_ability()
    {
        $user1 = User::create();
        $user2 = User::create();
        ...
        $bouncer->allow($user1)->to('*', User::class);

        $this->assertTrue($bouncer->denies('edit'));
        $this->assertTrue($bouncer->allows('edit', User::class));
        $this->assertTrue($bouncer->allows('edit', $user2));

        $bouncer->disallow($user1)->to('*', User::class);
        $this->clipboard->refresh();
    ...
    public function test_individual_model_wildcard_ability()
    {
        $user1 = User::create();
        $user2 = User::create();
        ...
        $bouncer->disallow($user1)->to('*', $user1);
        $this->clipboard->refresh();

        $this->assertTrue($bouncer->allows('edit', $user2));

        $bouncer->disallow($user1)->to('*', $user2);
        $this->clipboard->refresh();
