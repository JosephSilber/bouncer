...
Bouncer provides a mechanism to handle roles and abilities in [Laravel's ACL](http://laravel.com/docs/authorization). With an expressive and fluent syntax, it stays out of your way as much as possible: use it when you want, ignore it when you don't.
...
For more information about Laravel Facades, refer to [the Laravel documentation](https://laravel.com/docs/facades).
...
Authorizing users is handled directly at [Laravel's `Gate`](https://laravel.com/docs/authorization#gates), or on the user model (`$user->can($ability)`).
